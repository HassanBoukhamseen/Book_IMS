...
# HERE YOU WORK
@app.get("/authors")
def get_authors(current_user: dict = Depends(get_current_user)):
    if not current_user:
        return HTTPException(status_code=403, detail="Invalid Authorization")
    return retrieve_authors_from_db()

@app.get("/authors/{author_id}")
def get_author(author_id: int, current_user: dict = Depends(get_current_user)):
    if not current_user:
        return HTTPException(status_code=403, detail="Invalid Authorization")
...
#@ADMIN ONLY
@app.post("/authors")
def add_author(author: Author, current_user: dict = Depends(get_current_user)):
...
def update_author(author_id:int, new_author:AuthorUpdateCurrent, current_user: dict = Depends(get_current_user)):
    if not current_user or current_user["role"] != 1:
        return HTTPException(status_code=403, detail="Not autherized")
...
def delete_author(author_id: int, current_user: dict = Depends(get_current_user)):
    if not current_user or current_user["role"] != 1:
        return HTTPException(status_code=403, detail="Not autherized")
...
@app.put("/users/me")
async def update_user(user_update: UserUpdateCurrent, current_user: dict = Depends(get_current_user)):
